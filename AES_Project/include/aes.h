#ifndef AES_H
#define AES_H

#include <stdio.h>
#include <stdlib.h>
#include <string.h>

#define BLOCK_SIZE 16
#define KEY_SIZE 16
#define Nb 4
#define Nk 4
#define Nr 10
#define EXPANDED_KEY_SIZE 176

typedef unsigned char byte;

static FILE *log_file;

static const byte SBOX[256] = {
    
};

static const byte INV_SBOX[256] = {
    
};

static const byte RCON[10] = {0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36};

static byte galois_mult(byte a, byte b) {
    byte result = 0;
    for (int i = 0; i < 8; i++) {
        if (b & 1) result ^= a;
        byte high_bit = a & 0x80;
        a <<= 1;
        if (high_bit) a ^= 0x1b;
        b >>= 1;
    }
    return result;
}

static void log_matrix(const byte matrix[4][4], const char *step) {
    fprintf(log_file, "%s:\n", step);
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++)
            fprintf(log_file, "%02x ", matrix[i][j]);
        fprintf(log_file, "\n");
    }
    fprintf(log_file, "\n");
}

static void apply_sbox(byte matrix[4][4]) {
    for (int i = 0; i < 4; i++)
        for (int j = 0; j < 4; j++)
            matrix[i][j] = SBOX[matrix[i][j]];
    log_matrix(matrix, "SubBytes");
}

static void apply_inv_sbox(byte matrix[4][4]) {
    for (int i = 0; i < 4; i++)
        for (int j = 0; j < 4; j++)
            matrix[i][j] = INV_SBOX[matrix[i][j]];
    log_matrix(matrix, "InvSubBytes");
}

static void rotate_rows(byte matrix[4][4]) {
    byte temp;
    temp = matrix[1][0]; matrix[1][0] = matrix[1][1]; matrix[1][1] = matrix[1][2]; matrix[1][2] = matrix[1][3]; matrix[1][3] = temp;
    temp = matrix[2][0]; matrix[2][0] = matrix[2][2]; matrix[2][2] = temp;
    temp = matrix[2][1]; matrix[2][1] = matrix[2][3]; matrix[2][3] = temp;
    temp = matrix[3][0]; matrix[3][0] = matrix[3][3]; matrix[3][3] = matrix[3][2]; matrix[3][2] = matrix[3][1]; matrix[3][1] = temp;
    log_matrix(matrix, "ShiftRows");
}

static void inv_rotate_rows(byte matrix[4][4]) {
    byte temp;
    temp = matrix[1][3]; matrix[1][3] = matrix[1][2]; matrix[1][2] = matrix[1][1]; matrix[1][1] = matrix[1][0]; matrix[1][0] = temp;
    temp = matrix[2][0]; matrix[2][0] = matrix[2][2]; matrix[2][2] = temp;
    temp = matrix[2][1]; matrix[2][1] = matrix[2][3]; matrix[2][3] = temp;
    temp = matrix[3][0]; matrix[3][0] = matrix[3][1]; matrix[3][1] = matrix[3][2]; matrix[3][2] = matrix[3][3]; matrix[3][3] = temp;
    log_matrix(matrix, "InvShiftRows");
}

static void mix_columns(byte matrix[4][4]) {
    byte temp[4];
    for (int i = 0; i < 4; i++) {
        temp[0] = galois_mult(0x02, matrix[0][i]) ^ galois_mult(0x03, matrix[1][i]) ^ matrix[2][i] ^ matrix[3][i];
        temp[1] = matrix[0][i] ^ galois_mult(0x02, matrix[1][i]) ^ galois_mult(0x03, matrix[2][i]) ^ matrix[3][i];
        temp[2] = matrix[0][i] ^ matrix[1][i] ^ galois_mult(0x02, matrix[2][i]) ^ galois_mult(0x03, matrix[3][i]);
        temp[3] = galois_mult(0x03, matrix[0][i]) ^ matrix[1][i] ^ matrix[2][i] ^ galois_mult(0x02, matrix[3][i]);
        for (int j = 0; j < 4; j++)
            matrix[j][i] = temp[j];
    }
    log_matrix(matrix, "MixColumns");
}

static void inv_mix_columns(byte matrix[4][4]) {
    byte temp[4];
    for (int i = 0; i < 4; i++) {
        temp[0] = galois_mult(0x0e, matrix[0][i]) ^ galois_mult(0x0b, matrix[1][i]) ^ galois_mult(0x0d, matrix[2][i]) ^ galois_mult(0x09, matrix[3][i]);
        temp[1] = galois_mult(0x09, matrix[0][i]) ^ galois_mult(0x0e, matrix[1][i]) ^ galois_mult(0x0b, matrix[2][i]) ^ galois_mult(0x0d, matrix[3][i]);
        temp[2] = galois_mult(0x0d, matrix[0][i]) ^ galois_mult(0x09, matrix[1][i]) ^ galois_mult(0x0e, matrix[2][i]) ^ galois_mult(0x0b, matrix[3][i]);
        temp[3] = galois_mult(0x0b, matrix[0][i]) ^ galois_mult(0x0d, matrix[1][i]) ^ galois_mult(0x09, matrix[2][i]) ^ galois_mult(0x0e, matrix[3][i]);
        for (int j = 0; j < 4; j++)
            matrix[j][i] = temp[j];
    }
    log_matrix(matrix, "InvMixColumns");
}

static void xor_round_key(byte matrix[4][4], const byte round_key[16]) {
    for (int i = 0; i < 4; i++)
        for (int j = 0; j < 4; j++)
            matrix[i][j] ^= round_key[i + 4 * j]; // Column-major
    log_matrix(matrix, "AddRoundKey");
}

static void rotate_word(byte word[4]) {
    byte temp = word[0];
    word[0] = word[1];
    word[1] = word[2];
    word[2] = word[3];
    word[3] = temp;
}

static void substitute_word(byte word[4]) {
    for (int i = 0; i < 4; i++)
        word[i] = SBOX[word[i]];
}

static void expand_key(const byte key[KEY_SIZE], byte expanded_key[EXPANDED_KEY_SIZE]) {
    for (int i = 0; i < KEY_SIZE; i++)
        expanded_key[i] = key[i];

    byte temp[4];
    int bytes_done = KEY_SIZE;
    int rcon_idx = 0;

    while (bytes_done < EXPANDED_KEY_SIZE) {
        for (int i = 0; i < 4; i++)
            temp[i] = expanded_key[bytes_done - 4 + i];

        if (bytes_done % KEY_SIZE == 0) {
            rotate_word(temp);
            substitute_word(temp);
            temp[0] ^= RCON[rcon_idx++];
        }

        for (int i = 0; i < 4; i++) {
            expanded_key[bytes_done] = expanded_key[bytes_done - KEY_SIZE] ^ temp[i];
            bytes_done++;
        }
    }
}

static int pad_data(byte *data, int len) {
    int pad_len = BLOCK_SIZE - (len % BLOCK_SIZE);
    for (int i = len; i < len + pad_len; i++)
        data[i] = (byte)pad_len;
    return len + pad_len;
}

static int unpad_data(byte *data, int len) {
    if (len < BLOCK_SIZE) return len;
    int pad_len = data[len - 1];
    if (pad_len > BLOCK_SIZE || pad_len == 0) return len;
    for (int i = len - pad_len; i < len; i++)
        if (data[i] != pad_len) return len;
    return len - pad_len;
}

void encrypt_block(byte matrix[4][4], const byte expanded_key[EXPANDED_KEY_SIZE]) {
    byte round_key[16];

    for (int i = 0; i < 16; i++)
        round_key[i] = expanded_key[i];
    xor_round_key(matrix, round_key);

    for (int round = 1; round < Nr; round++) {
        apply_sbox(matrix);
        rotate_rows(matrix);
        mix_columns(matrix);
        for (int i = 0; i < 16; i++)
            round_key[i] = expanded_key[round * 16 + i];
        xor_round_key(matrix, round_key);
    }

    apply_sbox(matrix);
    rotate_rows(matrix);
    for (int i = 0; i < 16; i++)
        round_key[i] = expanded_key[Nr * 16 + i];
    xor_round_key(matrix, round_key);
}

void decrypt_block(byte matrix[4][4], const byte expanded_key[EXPANDED_KEY_SIZE]) {
    byte round_key[16];

    for (int i = 0; i < 16; i++)
        round_key[i] = expanded_key[Nr * 16 + i];
    xor_round_key(matrix, round_key);

    for (int round = Nr - 1; round > 0; round--) {
        inv_rotate_rows(matrix);
        apply_inv_sbox(matrix);
        for (int i = 0; i < 16; i++)
            round_key[i] = expanded_key[round * 16 + i];
        xor_round_key(matrix, round_key);
        inv_mix_columns(matrix);
    }

    inv_rotate_rows(matrix);
    apply_inv_sbox(matrix);
    for (int i = 0; i < 16; i++)
        round_key[i] = expanded_key[i];
    xor_round_key(matrix, round_key);
}

static void process_file(const char *input_path, const char *output_path, const char *log_path,
                         const byte key[KEY_SIZE], void (*process_block)(byte[4][4], const byte[EXPANDED_KEY_SIZE])) {
    FILE *in_file = fopen(input_path, "rb");
    FILE *out_file = fopen(output_path, "wb");
    log_file = fopen(log_path, "w");

    if (!in_file || !out_file || !log_file) {
        printf("Error opening files: %s, %s, or %s\n", input_path, output_path, log_path);
        if (in_file) fclose(in_file);
        if (out_file) fclose(out_file);
        if (log_file) fclose(log_file);
        return;
    }

    byte expanded_key[EXPANDED_KEY_SIZE];
    expand_key(key, expanded_key);

    byte block[BLOCK_SIZE];
    byte matrix[4][4];
    int data_len;

    fseek(in_file, 0, SEEK_END);
    data_len = ftell(in_file);
    fseek(in_file, 0, SEEK_SET);

    if (process_block == encrypt_block) {
        while (data_len >= BLOCK_SIZE) {
            fread(block, 1, BLOCK_SIZE, in_file);
            data_len -= BLOCK_SIZE;

            for (int i = 0; i < 4; i++)
                for (int j = 0; j < 4; j++)
                    matrix[j][i] = block[i * 4 + j]; // Column-major

            log_matrix(matrix, "Input Matrix");
            process_block(matrix, expanded_key);

            for (int i = 0; i < 4; i++)
                for (int j = 0; j < 4; j++)
                    block[i * 4 + j] = matrix[j][i];

            fwrite(block, 1, BLOCK_SIZE, out_file);
        }

        if (data_len > 0 || ftell(in_file) == 0) {
            memset(block, 0, BLOCK_SIZE);
            fread(block, 1, data_len, in_file);
            data_len = pad_data(block, data_len);

            for (int i = 0; i < 4; i++)
                for (int j = 0; j < 4; j++)
                    matrix[j][i] = block[i * 4 + j];

            log_matrix(matrix, "Padded Input Matrix");
            process_block(matrix, expanded_key);

            for (int i = 0; i < 4; i++)
                for (int j = 0; j < 4; j++)
                    block[i * 4 + j] = matrix[j][i];

            fwrite(block, 1, BLOCK_SIZE, out_file);
        }
    } else { // Decrypt
        byte *buffer = NULL;
        int buffer_len = 0;

        // Read entire file to buffer
        buffer = malloc(data_len);
        if (!buffer) {
            printf("Memory allocation failed\n");
            fclose(in_file);
            fclose(out_file);
            fclose(log_file);
            return;
        }
        fread(buffer, 1, data_len, in_file);
        buffer_len = data_len;

        // Process blocks
        for (int pos = 0; pos < buffer_len; pos += BLOCK_SIZE) {
            for (int i = 0; i < BLOCK_SIZE; i++)
                block[i] = buffer[pos + i];

            for (int i = 0; i < 4; i++)
                for (int j = 0; j < 4; j++)
                    matrix[j][i] = block[i * 4 + j];

            log_matrix(matrix, "Input Matrix");
            process_block(matrix, expanded_key);

            for (int i = 0; i < 4; i++)
                for (int j = 0; j < 4; j++)
                    block[i * 4 + j] = matrix[j][i];

            if (pos + BLOCK_SIZE >= buffer_len) {
                int new_len = unpad_data(block, BLOCK_SIZE);
                fwrite(block, 1, new_len, out_file);
            } else {
                fwrite(block, 1, BLOCK_SIZE, out_file);
            }
        }

        free(buffer);
    }

    fclose(in_file);
    fclose(out_file);
    fclose(log_file);
}

#endif